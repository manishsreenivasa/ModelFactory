...
if isempty(customLengthsDescription)
   disp(' - No segment lengths found for model. In order to use Jensen1986 scaling, segment lengths MUST be provided');
   error(' - Incompatible scaling algorithm chosen');
end
% Check if all required anthropometry has been defined
if isempty(inParam_PersonInfo.age) ...
        || isempty(inParam_PersonInfo.weight)...
        || isempty(inParam_PersonInfo.gender) ...
        || isempty(inParam_PersonInfo.pelvisWidth)...
        || isempty(inParam_PersonInfo.hipCenterWidth) ...
        || isempty(inParam_PersonInfo.shoulderCenterWidth)...
        || isempty(inParam_PersonInfo.heelAnkleXOffset) ...
        || isempty(inParam_PersonInfo.heelAnkleZOffset)...
        || isempty(inParam_PersonInfo.shoulderNeckZOffset) ...
        || isempty(inParam_PersonInfo.footWidth)
    disp ('  Check that the scaling type you are trying to use (fnc_scalingAlgo_Jensen1986) has all the information needed in the anthropometry file');
